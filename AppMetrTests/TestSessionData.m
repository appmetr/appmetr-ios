...
- (void)testBatchIndex {
    ...
    XCTAssertEqual(firstValue, secondValue, @"Wrong batch ID");
    [sessionData release];

    sessionData = [[SessionData alloc] init];
    NSUInteger finalValue = [sessionData nextBatchIndex];
    XCTAssertEqual(firstValue + 1, finalValue, @"Wrong batch ID restored");
...
- (void)testFileIndex {
    ...
    XCTAssertEqual(firstValue + 1, secondValue, @"Wrong file index");
    [sessionData release];

    sessionData = [[SessionData alloc] init];
    NSUInteger finalValue = [sessionData nextFileIndex];
    XCTAssertEqual(firstValue + 2, finalValue, @"Wrong file indes rrestored");
...
- (void)testFileList {
    ...
    XCTAssertEqual((NSUInteger) 0, sessionData2.fileList.count,
    ...
    XCTAssertTrue([fileName isEqualToString:@"testFiliList"],
